...
class App:
    ...
    def __init__(self):
        ...
        points_button = Button(self.frame, text="Selectionner les points", font=("Courrier", 25), bg='white',
                               fg='#41B77F',
                               command=self.points_selection)
        points_button.pack(pady=25, fill=X)
        # The button for predicting
        predict_button = Button(self.frame, text="Prédire", command=self.predict)
        predict_button.pack()
        # The button for choosing the image
        img_choice_button = Button(self.frame, text="Load une image", command=self.img_choice)
    ...
    def predict(self):
        ...
        fig, ax = plt.subplots()
        plt.imshow(color_enhanced(self.img_gray, self.gt_mask, masks[0]), cmap='gray')
    ...
    def points_selection(self):
        ...
        img_deepcopy = copy.deepcopy(self.img)

        cv2.namedWindow(window_name, cv2.WINDOW_KEEPRATIO)

        # displaying the image
        cv2.imshow(window_name, img_deepcopy)

        # setting mouse handler for the image and calling the click_event() function
        cv2.setMouseCallback(window_name, self.click_event, param={'window_name': window_name, 'image': img_deepcopy})
